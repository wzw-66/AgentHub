...
- [x] 8.1 实现消息意图分析，用于任务拆解
- [x] 8.2 实现并行任务分发，通过独立 SSE 流同时调用多个 Agent
- [x] 8.3 实现串行任务分发，带依赖链处理
- [x] 8.4 实现失败处理：重试一次后跳过
- [x] 8.5 实现所有任务完成后的聚合结果汇总
- [x] 8.6 编写任务分发策略的单元测试
...
- [x] 10.1 使用 Next.js App Router 创建 `apps/web` 和基础布局
- [x] 10.2 实现登录和注册页面
- [x] 10.3 实现三栏聊天布局（侧边栏、聊天面板、右侧面板）
- [x] 10.4 实现侧边栏：用户信息、搜索框、会话列表、新建聊天按钮
- [x] 10.5 实现聊天面板：头部、消息列表、输入框
- [x] 10.6 实现消息气泡渲染，按发送者区分样式
- [x] 10.7 实现聊天输入框：文本区域和发送按钮
- [x] 10.8 实现群聊 @提及弹出框
- [x] 10.9 实现 Agent 流式响应的输入中状态指示器
- [x] 10.10 集成 SSE 客户端，用于实时消息流
- [x] 10.11 集成 WebSocket 客户端，用于在线状态和通知

## 11. Agent 市场界面

- [x] 11.1 实现 Agent 列表页面（`/agents`），显示头像、名称、提供商
- [x] 11.2 实现 Agent 详情页面（`/agents/:id`），包含开始聊天操作
- [x] 11.3 实现自定义 Agent 创建表单
- [x] 11.4 实现联系人管理界面（添加、重命名、置顶/取消置顶）
